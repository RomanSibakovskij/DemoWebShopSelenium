...
public class TestMethods extends BaseTest{
    ...
    //'Payment Information' section too short / too long input

    //invalid guest checkout confirmation test method ('Credit Card' - too short cardholder name) (4 chars)
    protected void invalidGuestCheckoutCreditCardTooShortCardNameConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        //payment information web element assert //Selenium fails to find these elements with VALID selectors
        //isPaymentInformationSectionWebElementDisplayed(checkoutPage);
        //invalid guest credit card data getter (too short cardholder name)
        checkoutPaymentMethodPage.invalidGuestCreditCardTooShortCardNameDataGetter();
        //input too short guest cardholder name (4 chars)
        checkoutPaymentMethodPage.inputTooShortCreditCardNameIntoNameInputField();
        //input valid user credit card number
        checkoutPaymentMethodPage.inputValidCreditCardNumberIntoCardNumberInputField();
        //click credit card month dropdown menu
        checkoutPaymentMethodPage.clickCreditCardMonthDropdownMenu();
        ...
        //input valid user credit card CVC number
        checkoutPaymentMethodPage.inputValidCreditCardCVCIntoCVCInputField();
        //capture screenshot once the input is done
        captureScreenshot(driver, "Too short guest cardholder name");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears, if it doesn't, log the issue
        try {
            assertEquals("User cardholder name is too short.", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message text doesn't match expectations.");
        } catch (TimeoutException e) {
            // log the issue if error message didn't appear without crashing the test
            logger.error("The cardholder name input field doesn't have an error on 'too short' input.");
        }
        //capture screenshot at the end of the test (too short cardholder name)
        captureScreenshot(driver, "Invalid Guest Payment Information Test Result (with 'Credit Card' payment method - too short cardholder name)");
    }
    //invalid guest checkout confirmation test method ('Credit Card' - too long cardholder name) (4 chars)
    protected void invalidGuestCheckoutCreditCardTooLongCardNameConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        //payment information web element assert //Selenium fails to find these elements with VALID selectors
        //isPaymentInformationSectionWebElementDisplayed(checkoutPage);
        //invalid guest credit card data getter (too long cardholder name)
        checkoutPaymentMethodPage.invalidGuestCreditCardTooLongCardNameDataGetter();
        //input too short guest cardholder name (100 chars)
        checkoutPaymentMethodPage.inputTooLongCreditCardNameIntoNameInputField();
        //input valid user credit card number
        checkoutPaymentMethodPage.inputValidCreditCardNumberIntoCardNumberInputField();
        //click credit card month dropdown menu
        checkoutPaymentMethodPage.clickCreditCardMonthDropdownMenu();
        ...
        checkoutPaymentMethodPage.inputValidCreditCardCVCIntoCVCInputField();
        //capture screenshot once the input is done
        captureScreenshot(driver, "Too long guest cardholder name");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears, if it doesn't, log the issue
        try {
            assertEquals("User cardholder name is too long.", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message text doesn't match expectations.");
        } catch (TimeoutException e) {
            // log the issue if error message didn't appear without crashing the test
            logger.error("The cardholder name input field doesn't have an error on 'too long' input.");
        }
        //capture screenshot at the end of the test (too long cardholder name)
        captureScreenshot(driver, "Invalid Guest Payment Information Test Result (with 'Credit Card' payment method - too long cardholder name)");
    }
    //invalid input format
    //invalid guest checkout confirmation test method ('Credit Card' - invalid cardholder name format) (digits only)
    protected void invalidGuestCheckoutCreditCardInvalidCardNameFormatConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        //payment information web element assert //Selenium fails to find these elements with VALID selectors
        //isPaymentInformationSectionWebElementDisplayed(checkoutPage);
        //click credit card type dropdown menu
        checkoutPaymentMethodPage.clickCreditCardDropdownMenu();
        //select 'MasterCard'
        checkoutPaymentMethodPage.selectMasterCardOption();
        //invalid guest credit card data getter (invalid cardholder name format)
        checkoutPaymentMethodPage.invalidGuestCreditCardInvalidCardNameFormatDataGetter();
        //input invalid guest cardholder name (by format) (digits only)
        checkoutPaymentMethodPage.inputInvalidCreditCardNameFormatIntoNameInputField();
        //input valid user credit card number
        checkoutPaymentMethodPage.inputValidCreditCardNumberIntoCardNumberInputField();
        //click credit card month dropdown menu
        checkoutPaymentMethodPage.clickCreditCardMonthDropdownMenu();
        ...
        checkoutPaymentMethodPage.inputValidCreditCardCVCIntoCVCInputField();
        //capture screenshot once the input is done
        captureScreenshot(driver, "Invalid guest cardholder name format");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears, if it doesn't, log the issue
        try {
            assertEquals("User cardholder name format is invalid.", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message text doesn't match expectations.");
        } catch (TimeoutException e) {
            // log the issue if error message didn't appear without crashing the test
            logger.error("The cardholder name input field doesn't have an error on 'invalid format' input.");
        }
        //capture screenshot at the end of the test (invalid cardholder name format)
        captureScreenshot(driver, "Invalid Guest Payment Information Test Result (with 'Credit Card' payment method - invalid cardholder name format)");
    }

    //invalid guest checkout confirmation test method ('Credit Card' - too short card number)
    protected void invalidGuestCheckoutCreditCardTooShortCardNumberConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        //select 'MasterCard'
        checkoutPaymentMethodPage.selectMasterCardOption();
        //invalid guest credit card data getter (too short card number)
        checkoutPaymentMethodPage.invalidGuestCreditCardTooShortCardNumberDataGetter();
        //input valid guest cardholder name
        checkoutPaymentMethodPage.inputValidCreditCardNameIntoNameInputField();
        //input too short credit card number (15 digits)
        checkoutPaymentMethodPage.inputTooShortCreditCardNumberIntoCardNumberInputField();
        //click credit card month dropdown menu
        checkoutPaymentMethodPage.clickCreditCardMonthDropdownMenu();
        //select valid month
        checkoutPaymentMethodPage.selectCreditCardValidMonthOption();
        ...
        checkoutPaymentMethodPage.inputValidCreditCardCVCIntoCVCInputField();
        //capture screenshot once the input is done
        captureScreenshot(driver, "Too short guest card number");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears
        assertEquals("Wrong card number", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message doesn't match expectations.");
        //capture screenshot at the end of the test (capture the result)
        captureScreenshot(driver, "Invalid Guest Payment Information Test Result (with 'Credit Card' payment method - too short credit card number)");
    }
    //invalid guest checkout confirmation test method ('Credit Card' - too long card number)
    protected void invalidGuestCheckoutCreditCardTooLongCardNumberConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        //select 'MasterCard'
        checkoutPaymentMethodPage.selectMasterCardOption();
        //invalid guest credit card data getter (too long card number)
        checkoutPaymentMethodPage.invalidGuestCreditCardTooLongCardNumberDataGetter();
        //input valid guest cardholder name
        checkoutPaymentMethodPage.inputValidCreditCardNameIntoNameInputField();
        //input too short credit card number (18 digits)
        checkoutPaymentMethodPage.inputTooLongCreditCardNumberIntoCardNumberInputField();
        //click credit card month dropdown menu
        checkoutPaymentMethodPage.clickCreditCardMonthDropdownMenu();
        //select valid month
        checkoutPaymentMethodPage.selectCreditCardValidMonthOption();
        ...
        checkoutPaymentMethodPage.inputValidCreditCardCVCIntoCVCInputField();
        //capture screenshot once the input is done
        captureScreenshot(driver, "Too long guest card number");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears
        assertEquals("Wrong card number", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message doesn't match expectations.");
        //capture screenshot at the end of the test (capture the result)
        captureScreenshot(driver, "Invalid Guest Payment Information Test Result (with 'Credit Card' payment method - too long credit card number)");
    }
    // invalid input format
    //invalid guest checkout confirmation test method ('Credit Card' - invalid card number format)
    protected void invalidGuestCheckoutCreditCardInvalidCardNumberFormatConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        checkoutPaymentMethodPage.clickCreditCardDropdownMenu();
        //select 'Amex'
        checkoutPaymentMethodPage.selectAmexOption();
        //invalid guest credit card data getter (invalid card number format)
        checkoutPaymentMethodPage.invalidGuestCreditCardInvalidCardNumberFormatDataGetter();
        //input valid guest cardholder name
        checkoutPaymentMethodPage.inputValidCreditCardNameIntoNameInputField();
        //input invalid credit card number (by format) (random 16 digits - no structure)
        checkoutPaymentMethodPage.inputInvalidCreditCardNumberFormatIntoCardNumberInputField();
        //click credit card month dropdown menu
        checkoutPaymentMethodPage.clickCreditCardMonthDropdownMenu();
        //select valid month
        checkoutPaymentMethodPage.selectCreditCardValidMonthOption();
        ...
        checkoutPaymentMethodPage.inputValidCreditCardCVCIntoCVCInputField();
        //capture screenshot once the input is done
        captureScreenshot(driver, "Invalid guest card number format");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears
        assertEquals("Wrong card number", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message doesn't match expectations.");
        //capture screenshot at the end of the test (capture the result)
        captureScreenshot(driver, "Invalid Guest Payment Information Test Result (with 'Credit Card' payment method - invalid credit card number format)");
    }

    //invalid guest checkout confirmation test method ('Credit Card' - too short card CVC number)
    protected void invalidGuestCheckoutCreditCardTooShortCardCVCNumberConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        checkoutPaymentMethodPage.clickCreditCardDropdownMenu();
        //select 'Discover'
        checkoutPaymentMethodPage.selectDiscoverOption();
        //invalid guest credit card data getter (too short card CVC number)
        checkoutPaymentMethodPage.invalidGuestCreditCardTooShortCardCVCNumberDataGetter();
        //input valid guest cardholder name
        checkoutPaymentMethodPage.inputValidCreditCardNameIntoNameInputField();
        //input valid guest credit card number
        checkoutPaymentMethodPage.inputValidCreditCardNumberIntoCardNumberInputField();
        ...
        //select valid year
        checkoutPaymentMethodPage.selectCreditCardValidYearOption();
        //input too short credit card CVC number (2 digits)
        checkoutPaymentMethodPage.inputTooShortCreditCardCVCIntoCVCInputField();
        //capture screenshot once the input is done
        captureScreenshot(driver, "Too short guest card CVC number");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears
        assertEquals("Wrong card code", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message doesn't match expectations.");
    ...
    // invalid input format
    //invalid guest checkout confirmation test method ('Credit Card' - invalid card CVC number format - chars only)
    protected void invalidGuestCheckoutCreditCardInvalidCardCVCFormatConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        //select 'Amex'
        checkoutPaymentMethodPage.selectAmexOption();
        //invalid guest credit card data getter (invalid card CVC number format)
        checkoutPaymentMethodPage.invalidGuestCreditCardInvalidCardCVCFormatDataGetter();
        //input valid guest cardholder name
        checkoutPaymentMethodPage.inputValidCreditCardNameIntoNameInputField();
        //input valid guest credit card number
        checkoutPaymentMethodPage.inputValidCreditCardNumberIntoCardNumberInputField();
        ...
        //select valid year
        checkoutPaymentMethodPage.selectCreditCardValidYearOption();
        //input invalid credit card CVC number format (chars only)
        checkoutPaymentMethodPage.inputInvalidCreditCardCVCFormatIntoCVCInputField();
        //capture screenshot once the input is done
        captureScreenshot(driver, "Invalid guest card CVC number format");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears
        assertEquals("Wrong card code", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message doesn't match expectations.");
        //capture screenshot at the end of the test (capture the result)
        captureScreenshot(driver, "Invalid Guest Payment Information Test Result (with 'Credit Card' payment method - invalid credit card CVC format)");
    }

    //invalid guest checkout confirmation test method ('Credit Card' - expired credit card)
    protected void invalidGuestCheckoutExpiredCreditCardConfirmationTest(){
        CheckoutPage checkoutPage = new CheckoutPage(driver);
        CheckoutGuestDataPage checkoutGuestDataPage = new CheckoutGuestDataPage(driver);
        CheckoutPaymentMethodPage checkoutPaymentMethodPage = new CheckoutPaymentMethodPage(driver);
        //checkout page billing address web element assert
        ...
        //click credit card year dropdown menu
        checkoutPaymentMethodPage.clickCreditCardYearDropdownMenu();
        //select expired year (for expired date scenario)
        checkoutPaymentMethodPage.selectCreditCardExpiredYearOption();
        //input valid user credit card CVC number
        checkoutPaymentMethodPage.inputValidCreditCardCVCIntoCVCInputField();
        //capture screenshot at payment information section (for verification)
        captureScreenshot(driver, "Expired credit card");
        //click 'Continue' button (payment information)
        checkoutPage.clickPaymentInfoContinueButton();
        //assert the expected error message appears, if it doesn't, log the issue
        try {
            assertEquals("User card is expired.", checkoutPaymentMethodPage.getMissingCreditCardInputErrorMessage(), "The error message text doesn't match expectations.");
        } catch (TimeoutException e) {
            // log the issue if error message didn't appear without crashing the test
            logger.error("The application doesn't seem to verify whether the user card has expired or not.");
        }
        //capture screenshot at the end of the test (expired credit card)
